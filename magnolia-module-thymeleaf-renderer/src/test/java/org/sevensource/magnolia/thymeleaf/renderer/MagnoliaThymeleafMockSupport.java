...
public class MagnoliaThymeleafMockSupport {
	...
	public void before() throws Exception {
		...
		when(componentProvider.newInstance(eq(PageElement.class), any())).then((i) -> {
			...
			return new PageElement(serverConfiguration, renderingContext, templatingModuleProvider, webContext);
		});
		...
		when(componentProvider.newInstance(eq(ComponentElement.class), any())).then((i) -> {
			...
			return new ComponentElement(serverConfiguration, renderingContext, engine, templateDefinitionAssignment, webContextProvider.get(), templatingModuleProvider);
		});
